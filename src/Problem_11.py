...
def calProduct(num,n1,n2,n3,n4):
    temp=int(n1)*int(n2)*int(n3)*int(n4)
    if num<temp:
        return temp
    return num
...
for l in range(len(arr)): 
    for x in range(len(arr[l])):
        if l > 2:
            maxP=calProduct(maxP,arr[l][x],arr[l-1][x],arr[l-2][x], arr[l-3][x]) 
            if x > 2:
                maxP=calProduct(maxP,arr[l][x],arr[l-1][x-1],arr[l-2][x-2], arr[l-3][x-3]) 
            if x < len(arr[l])-3:
                maxP=calProduct(maxP,arr[l][x],arr[l-1][x+1],arr[l-2][x+2], arr[l-3][x+3]) 
        if x > 2:
            maxP=calProduct(maxP,arr[l][x],arr[l][x-1],arr[l][x-2], arr[l][x-3]) 
        if x < len(arr[l])-3:
            maxP=calProduct(maxP,arr[l][x],arr[l][x+1],arr[l][x+2], arr[l][x+3]) 
print maxP
